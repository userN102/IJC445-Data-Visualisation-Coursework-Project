...
# Extract enterprise birth counts by SIC industry and year
# Source: ONS Business Demography Excel file (ons_original.xlsx), Table 1.2
# Output: Long-format dataset (industry_code × year) for entry-rate calculation
...
# Read Table 1.2, skipping metadata rows added by ONS Excel formatting
df <- read_excel(file_path, sheet = "Table 1.2", skip = 3)
...
# - 2 digits correspond to division level (used for analysis)
...
# Reshape to long format to create an industry–year panel
df_long <- df %>%
  pivot_longer(
    cols = all_of(year_cols),
    names_to = "year",
    values_to = "births_of_new_enterprises"
  )

# Save output for merging with deaths, active enterprises, survival, and growth tables
write.csv(
  df_long,
  "./Dataset/ONS_Business_Demography/Births_Of_New_Enterprises_2019_2024_by_Industry.csv",
  row.names = FALSE
)
